...
\centering
\small
\caption{Architecture robustness of the spatial-vessel contribution. Each row adds the same frozen 1792-d vessel embedding to a different RGB representation, with its own matched RGB-only control; the two rows are independent experiments, not a single model comparison.}
